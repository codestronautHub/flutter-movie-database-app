...
class TvRepositoryImpl implements TvRepository {
  ...
  @override
  Future<Either<Failure, List<Tv>>> getTopTqTvs() async {
    try {
      final result = await remoteDataSource.getTopTqTvs();
      return Right(result.map((model) => model.toEntity()).toList());
    } on ServerException {
      return const Left(ServerFailure(''));
    } on SocketException {
  ...
  Future<Either<Failure, List<TvSeasonEpisode>>> getTvSeasonEpisodes(
    ...
    try {
      final result = await remoteDataSource.getTvSeasonEpisodes(id);
      return Right(result.map((model) => model.toEntity()).toList());
    } on ServerException {
      return const Left(ServerFailure(''));
    } on SocketException {
